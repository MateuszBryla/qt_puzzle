...
static int getSize(int argc, char *argv[])
{
    if (argc < 2)
    {
        std::cout << "[ERROR] Puzzle size wasn't specified, pass it as a single number, as a first argument\n";
        exit(0);
    }
...
int main(int argc, char *argv[])
{
    ...
    while(not puzzle.isFinished()){
        clearTerminal();
        std::cout << "Press W/S/A/D & Enter to move Up/Down/Left/Right, press Q and Enter to Exit" << std::endl;
        puzzle.printPuzzle();
        std::cin >> input;
        std::cout << input;
