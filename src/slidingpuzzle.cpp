...
{
  // Set two elements as equal - may be useful, if two blocks in image mode are
  // the same
...
bool SlidingPuzzle::isSolvable()
{
  ...
  // Inversion: when a higher-numbered tile precedes a lower-numbered tile in
  // the puzzle
  ...
  // For odd-sized puzzles (e.g., 3x3), the puzzle is solvable if the number of
  // inversions is even.
  if (mPuzzleSize % 2)
  {
    if (inversions % 2)
  ...
  // 1. The number of inversions is even, and the row of the blank tile
  // (starting from the bottom) is even.
  unsigned index = findZeroIndex(mPuzzleArr, mArraySize);
  ...
  // 2. The number of inversions is odd, and the row of the blank tile (starting
