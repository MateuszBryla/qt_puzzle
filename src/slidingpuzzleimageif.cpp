...
SlidingPuzzleImageIF::SlidingPuzzleImageIF(std::string image_path,
                                           unsigned size)
    : SlidingPuzzleImageIF(image_path, size, SlidingPuzzleImageIF::CROP) {}
...
void SlidingPuzzleImageIF::splitIntoPuzzles()
{
  ...
  // Create "blank puzzle"
  cv::Mat element =
      cv::Mat::zeros(cv::Size(elementWidth, elementHeight), CV_8UC3);
  ...
  for (int i = 0; i < mPuzzleSize; i++)
  {
    ...
    {
      cv::Rect roi(j * elementWidth, i * elementHeight, elementWidth,
                   elementHeight);
